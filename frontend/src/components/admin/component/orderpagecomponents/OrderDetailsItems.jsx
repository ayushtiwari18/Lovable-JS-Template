...
export function OrderDetailsItems({ items, customizationDetails }) {
  ...
  // Parse customizationDetails JSON string/object
      ...
      <CardContent>
        <div className="space-y-4">
          {formatted.data.map((item) => {
            // Use productId as key for customization lookup
            const customizationDetail = customizationObj[item.productId];

            const customizations = customizationDetail?.customizations || {};

            const { text, uploadedImage } = customizations;

            return (
              <div
                key={item.id} // key still safely on item.id, unique in list
              ...
              >
                ...
                {/* New customization details: text and uploaded image */}
                {(text || (uploadedImage && uploadedImage.url)) && (
